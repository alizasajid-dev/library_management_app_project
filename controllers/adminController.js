...
// DSA Concepts - Helper Functions

// Simple Linear Search
const linearSearch = (arr, searchTerm) => {
  for (let i = 0; i < arr.length; i++) {
    if (arr[i].toLowerCase().includes(searchTerm.toLowerCase())) {
      return i;
    }
  }
  return -1;
...
// Bubble Sort for Books (Sorting based on Title)
...
// Using Bubble Sort to Sort Books by Title
exports.books = (req, res) => {
  let currentPage = req.params.page || 1
  let perPage = req.query.perPage || 10
  let totalBook
  ...
  query
    ...
    .then(count => {
      ...
      return Book.find({ $or: [{ title: { $regex: req.query.search || '', $options: 'i' } }, { isbn: { $regex: req.query.search || '', $options: 'i' }}] })
        .skip(parseInt(currentPage - 1) * parseInt(perPage))
        .limit(parseInt(perPage))
        .sort({ title: 1 }) // Sorting books by title
    })
    .then(books => {
      // Using Bubble Sort for sorting books manually
      const sortedBooks = bubbleSort(books)
      res.render('admin/books', {
        books: sortedBooks,
        searchOption: req.query,
        currentPage: parseInt(currentPage),
